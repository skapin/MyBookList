...
* 
**/
function form_preped_text($label,$name,$prepend,$placeholder,$BSGrid,$size,$onchange,$type='text',$value='')
{
	set_post($name);
	
return	'<!-- Prepended text-->
<div class="form-group">
  <label class="sr-only col-md-'.$BSGrid.' control-label" for="'.$name.'">'.$label.'</label>
    <div class="input-group">
      <span class="input-group-addon">'.$prepend.'</span>
      <input id="'.$name.'" name="'.$name.'" class="form-control" placeholder="'.($placeholder!==''?$placeholder:post2text($_POST[$placeholder])).'" '.($size?'size="'.$size.'" 
maxlength="'.$size.'" ':'').''.($onchange?'onchange="'.$onchange.'" ':'').' type="'.$type.'">    
  </div>
</div>';
}
/**
 ...
 * 
 * */
function form_check_bs($label,$groupe,$name)
{
  return ($label?'<label for="'.$name.'" >'.$label.'     ':'').'<input type="checkbox" value="'.$name.'" name="'.$groupe.'" />'.($label?'</label>':'');
}

function form_check_as_button($label,$groupe,$name)
{
  return '<label><input type="checkbox" value="'.$name.'" name="'.$groupe.'" /><span>'.$label.'</span></label>';
}
 ...
 * 
 * */
function form_select_bs($label,$name,$options,$onchange='',$value='')
{
	set_post($name);
	$select=($label?'<label for="'.$name.'">'.$label.'</label>':'').'<select id="'.$name.'" class="combobox form-control" name="'.$name.'"'.($onchange?' onchange="'.$onchange.'"':'').'>
		';
